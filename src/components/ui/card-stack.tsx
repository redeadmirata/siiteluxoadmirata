...
export type CardStackProps<T extends CardStackItem> = {
  ...
  showDots?: boolean
  className?: string
  onClickCard?: (item: T, index: number) => void
  ...
  return (
    ...
    >
      <div
        ...
        tabIndex={0}
        onKeyDown={onKeyDown}
        aria-label="Galeria de fotos — use ← → para navegar"
      >
        {/* ambient glow */}
        <div
          className="pointer-events-none absolute inset-x-0 top-6 mx-auto h-40 w-[70%] rounded-full bg-black/5 blur-3xl"
          aria-hidden="true"
        />
        <div
          className="pointer-events-none absolute inset-x-0 bottom-0 mx-auto h-32 w-[76%] rounded-full bg-black/8 blur-3xl"
          aria-hidden="true"
        />
      ...
      <div className="mt-4 flex items-center justify-center gap-4">
            ...
            {items.map((it, idx) => (
              <button
                ...
                className={cn(
                  ...
                  idx === active ? 'w-6 bg-ink' : 'w-1.5 bg-ink/25 hover:bg-ink/50',
                )}
        ...
        <div className="flex gap-2">
          <button
            onClick={prev}
            className="w-8 h-8 rounded-full border border-ink/20 flex items-center justify-center text-ink/50 hover:text-ink hover:border-ink/50 transition-colors text-sm"
